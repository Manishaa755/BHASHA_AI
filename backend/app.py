...
# --------------------------------------------------------------------
# Validate API key dependency selection (with DEV_MODE permissive fallback)
# --------------------------------------------------------------------
...
# DEV_MODE permissive validator (local demo only)
DEV_MODE = os.getenv("DEV_MODE") in ("1", "true", "True")
if DEV_MODE:
    async def _dev_validate_api_key_header(x_api_key: Optional[str] = Header(None)):
        """
        Development/demo validator: accepts any non-empty X-API-Key and returns
        a simple api_key_info dict. DO NOT USE IN PRODUCTION.
        Enable by setting DEV_MODE=1 in your environment.
        """
        if not x_api_key or not str(x_api_key).strip():
            raise HTTPException(
                status_code=401, detail="Missing X-API-Key header (DEV_MODE).")
        return {"id": "dev-demo-key", "user_id": "dev", "email": "dev@example.com", "raw_key": x_api_key}
    validate_api_key_header = _dev_validate_api_key_header
    print("⚠️ Running in DEV_MODE: permissive X-API-Key validator enabled (DO NOT use in production).")

# Final fallback: clear error message instructing to implement validator
if not validate_api_key_header:
...
# --------------------------------------------------------------------
# Last-mile CORS + safety (with better exception logging)
# --------------------------------------------------------------------
...
async def ensure_cors_header(request: Request, call_next):
    try:
        ...
        print("❌ Unhandled exception during request handling:", repr(e))
        traceback.print_exc()
    ...
    try:
        ...
        print("⚠️ error while attaching CORS headers:", repr(e))
        traceback.print_exc()
    return resp
...
# NOTE: demo_proxy inclusion is now opt-in via ENABLE_DEMO_PROXY env var to avoid duplicate /demo/* registrations
try:
    if os.getenv("ENABLE_DEMO_PROXY", "").lower() in ("1", "true"):
        import demo_proxy
        if hasattr(demo_proxy, "router"):
            app.include_router(demo_proxy.router)
            print("✅ included demo_proxy.router (ENABLE_DEMO_PROXY enabled)")
    else:
        print("ℹ️ demo_proxy not included (set ENABLE_DEMO_PROXY=1 to opt-in).")
except Exception as e:
...
# ------------------ Demo proxy endpoints (frontend-friendly) ------------------
# These endpoints use the server's OPENAI_API_KEY and do not require any client-side key.
# They are intentionally simple and for demo/testing only. Remove or protect in production.

from fastapi import UploadFile  # ensure imported earlier (already in your file)

@app.post("/demo/chat_text")
async def demo_chat_text(payload: dict = Body(...)):
    """
    Simple text chat demo used by the landing page widget.
    Expects JSON: { "text": "..." }
    Returns: { "bot_reply": "..." }
    """
    try:
        ...
        if not text:
            return JSONResponse(status_code=400, content={"detail": "Missing 'text' in request body."})
        # Choose a target language: try to infer, fallback to en
        try:
            ens = await ensemble_best_code(text)
            default_code = ens["best"]["code_base"] or "en"
        except Exception:
            default_code = "en"
        tgt = await llm_infer_target_language(text, default_code)
        reply = await openai_chat_reply(text, tgt, enforce_lang=False)
        return JSONResponse({"bot_reply": reply})
    except HTTPException as he:
        return JSONResponse(status_code=he.status_code, content={"detail": he.detail})
    except Exception as e:
        print("❌ /demo/chat_text error:", repr(e))
        return JSONResponse(status_code=500, content={"error": "internal_server_error", "detail": str(e)[:800]})

@app.post("/demo/s2s")
async def demo_s2s(file: UploadFile = File(...), format: Optional[str] = Query("mp3")):
    """
    Demo speech->speech endpoint used by the landing page widget.
    Accepts multipart file upload (file) and returns TTS audio (mp3 by default).
    Uses server-side keys and the existing helpers: openai_transcribe, openai_chat_reply, openai_tts.
    """
    try:
        if not file or not file.filename:
        ...
        if d.get("intent") == "dict":
            ...
            # best-effort reply; use live web if needed
            if needs_web(user_text):
                web = await answer_with_web(user_text, target_lang=tgt_lang if tgt_lang != "auto" else "en")
                reply_text = web.get("answer") or ""
                if not reply_text:
                    reply_text = await openai_chat_reply(user_text, tgt_lang, enforce_lang=True)
            else:
                reply_text = await openai_chat_reply(user_text, tgt_lang, enforce_lang=True)
        ...
        audio_out = await openai_tts(reply_text=reply_text, voice=TTS_DEFAULT_VOICE, fmt=fmt, model=TTS_MODEL)
        media_type = _audio_content_type(fmt)

        headers = {
            "X-Reply-Text-B64": b64_header(reply_text),
            "X-Source-Text": b64_header(user_text),
            "X-Source-Lang": safe_header_value(src_lang, 32),
            "X-Target-Lang": safe_header_value(tgt_lang, 32),
            "Cache-Control": "no-store",
            "Content-Disposition": safe_header_value(f'inline; filename="reply.{fmt}"'),
